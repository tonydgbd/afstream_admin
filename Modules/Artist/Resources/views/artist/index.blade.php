        ...
        <div class="colxl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <div class="page-title-wrapper">
                <div class="page-title-box">
                    <h4 class="page-title bold">{{ __('adminWords.all').' '.__('adminWords.artist')}}</h4>
                </div>
				...
				<div class="card m-b-30">
					<div class="card-header">                                
						<div class="row align-items-center">
							...
							<div class="col-6">
								<div class="btn-right">
									<a class="effect-btn btn btn-primary mr-2" href="{{route('artist.create')}}"><i class="feather icon-plus mr-2"></i>{{ __('adminWords.create').' '.__('adminWords.artist') }}</a>
									<button type="button" class="effect-btn btn btn-danger" id="bulkDelete" data-msg="{{ __('adminWords.atleast').__('adminWords.artist').__('adminWords.must_selected') }}" data-url="{{route('artist.bulk_delete')}}"><i class="far fa-trash-alt mr-2 "></i> {{ __('adminWords.delete_selected') }}</button>   
								</div>
							</div>
						</div>
					</div>
					<div class="card-body">
	                        ...
	                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 adminMultipleFilterOption"> 
	                            ...
	                            <div class="data_tablefilter_option">                                                        
	                                ...
	                                <div class="form-group">
	                                    <label for="psArtistStatusData">{{ __('adminWords.select').' '.__('adminWords.artist').' '.__('adminWords.status') }}</label> 
	                                    <select class="form-control getArtistStatus" id="psArtistStatusData">
	                                        <option value="" disabled selected>--{{ __('adminWords.select').' '.__('adminWords.artist').' '.__('adminWords.status') }}--</option>	                                        
	                                        <option value="P">{{ __('adminWords.pending') }}</option>
	                                        <option value="A">{{ __('adminWords.approved') }}</option>
	                                        <option value="R">{{ __('adminWords.reject') }}</option>
	                                    </select>
	                                </div>
	                                ...
	                                <div class="data_tablefilter_btn">
	                                    <button type="button" class="effect-btn btn btn-primary multiple_dt_filter" data-tabel="artistDataTables" data-url="{{route('artistData')}}">{{ __('adminWords.filter') }}</button> 
	                                </div>
	                            </div>
						...
						<div class="table-responsive">
							<form method="post" id="artistForm">
								<table data-method="post" id="datatable-buttons" class="table table-styled musiooDtToShowData artistDataTables" data-url="{{route('artistData')}}">
									<thead>
										<tr> 
											...
											<th>{{ __('adminWords.balance') }}</th>
											<th>{{ __('adminWords.verify_status') }}</th>
