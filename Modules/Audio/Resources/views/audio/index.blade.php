			...
			<div class="card m-b-30">
				<div class="card-header">                                
					<div class="row align-items-center">
						...
						<div class="col-6">
							<div class="btn-right">
								<a class="effect-btn btn btn-primary mr-2" href="{{route('audio.create')}}"><i class="feather icon-plus mr-2"></i>{{ __('adminWords.create').' '.__('adminWords.audio') }}</a>
								<button type="button" class="effect-btn btn btn-danger" id="bulkDelete" data-msg="{{ __('adminWords.atleast').' '.__('adminWords.audio').' '.__('adminWords.must_selected') }}" data-url="{{route('bulkDeleteAudio')}}"><i class="far fa-trash-alt mr-2 "></i> {{ __('adminWords.delete_selected') }}</button>   
							</div>
						</div>
					</div>
				</div>
				<div class="card-body">
					<div class="table-responsive">
						<form method="post" id="audioForm">
							<table data-method="post" id="datatable-buttons" class="table table-styled musiooDtToShowData" data-url="{{route('audioData')}}">
								<thead>
									<tr> 
