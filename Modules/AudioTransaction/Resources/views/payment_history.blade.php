			...
			<div class="card m-b-30">
				<div class="card-header">                                
					<div class="row align-items-center">
						<div class="col-6">
							<h5 class="card-title mb-0">{{ __('adminWords.all').' '.__('adminWords.payment_history') }}</h5>
						</div>
					</div>
				</div>
                        ...
                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 adminMultipleFilterOption"> 
                            <div class="data_tablefilter_option">
                                <div class="form-group">
                                    <label for="phArtistData">{{ __('adminWords.select').' '.__('adminWords.artist') }}<sup>*</sup></label> 
                                    <select class="form-control select2WithSearch require getArtistId" id="phArtistData">
                                ...
                                <div class="data_tablefilter_btn">
                                    <button type="button" class="effect-btn btn btn-primary multiple_dt_filter" data-tabel="paymentHistoryDataTables" data-url="{{route('payment_history_data')}}">{{ __('adminWords.filter') }}</button>
                                </div>
                            </div>
                        </div>                 
                    </div>  
                    <div class="table-responsive">
                        <form method="post" id="paymentHistoryForm">
                            <table data-method="post" id="datatable-buttons" class="table table-styled musiooDtToShowData paymentHistoryDataTables" data-url="{{route('payment_history_data')}}">
                                <thead>
                                    <tr> 
                                        <th>#</th>
