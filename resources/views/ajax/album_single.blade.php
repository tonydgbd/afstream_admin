...
@php     
    ...
    $song_list = json_decode($album->song_list);        
@endphp
        ...
        <div class="ms_artist_wrapper common_pages_space">
            <div class="album_single_data">
                ...
                <div class="album_single_text">
                    ...
                    <p class="singer_name">{{ __('frontWords.by') }} - {{ $artist_name != '' ? rtrim($artist_name,',') : '' }}</p>
                    <div class="album_feature">
                        <a href="javascript:void(0)" class="album_date">{{ (!empty($song_list)) ? count($song_list) : 0 }} song | {{ multiple_audio_duration(['list'=>$album->song_list,'add'=>1]) }}</a>
                            ...
                            <ul class="ms_common_dropdown ms_downlod_list list_more">     
                                @if(!empty(auth()->user()->id))
                                    <li class="favourite_icon">
                                        <a href="javascript:void(0);" class="addToFavourite" data-favourite="{{ $album->id }}" data-type="album">
                                            @php
                                                $getData = \App\Favourite::where(['user_id'=> auth()->user()->id])->first();
                                                    if(!empty($getData->album_id)){
                                                        $decodeIds = $getData->album_id;  
                                                        if($decodeIds != '' && !empty($decodeIds)){
                                                            $dataId = json_decode($decodeIds);
                                                            if( in_array($album->id, $dataId) ) {
                                            ...
                                            @php            }
                                                        ...
                                                        echo'<svg xmlns:xlink="http://www.w3.org/1999/xlink" width="17px" height="16px"><path fill-rule="evenodd" fill="rgb(124, 142, 165)" d="M11.777,-0.000 C10.940,-0.000 10.139,0.197 9.395,0.585 C9.080,0.749 8.783,0.947 8.506,1.173 C8.230,0.947 7.931,0.749 7.618,0.585 C6.874,0.197 6.073,-0.000 5.236,-0.000 C2.354,-0.000 0.009,2.394 0.009,5.337 C0.009,7.335 1.010,9.428 2.986,11.557 C4.579,13.272 6.527,14.702 7.881,15.599 L8.506,16.012 L9.132,15.599 C10.487,14.701 12.436,13.270 14.027,11.557 C16.002,9.428 17.004,7.335 17.004,5.337 C17.004,2.394 14.659,-0.000 11.777,-0.000 ZM5.236,2.296 C6.168,2.296 7.027,2.738 7.590,3.507 L8.506,4.754 L9.423,3.505 C9.986,2.737 10.844,2.296 11.777,2.296 C13.403,2.296 14.727,3.660 14.727,5.337 C14.727,6.734 13.932,8.298 12.364,9.986 C11.114,11.332 9.604,12.490 8.506,13.255 C7.409,12.490 5.899,11.332 4.649,9.986 C3.081,8.298 2.286,6.734 2.286,5.337 C2.286,3.660 3.610,2.296 5.236,2.296 Z"/></svg>';                                                    
                                                    }
                                            @endphp
                                        </a>
                                ...
                                <li>
                                    <a href="javascript:void(0);" class="add_to_queue" data-musicid="{{ $album->id }}" data-musictype="album">
                                    <span class="opt_icon" title="Add To Queue"><span class="icon icon_queue"></span></span>
                                    {{ __("frontWords.add_to_queue") }}
                                    </a>
                                </li>
                                <li>
                                    <a href="javascript:void(0);" class="ms_share_music" data-shareuri="{{ url('album/single/'.$album->id.'/'.$album->album_slug) }}" data-sharename="{{ $album->album_name }}">
            ...
            <div class="album_list_wrapper">
                    ...
                    <ul>
                        ...
                        <li><a class="play_music" data-musicid="{{ $audios->id }}" data-musictype="audio" data-url="{{ url('/songs') }}" href="javascript:void(0)">{{ $audios->audio_title }}</a></li>
                        <li class="play_music" data-musicid="{{ $audios->id }}" data-musictype="audio" data-url="{{ url('/songs') }}">{{ rtrim($artist_name,',') }}</li>
                                    ...
                                    @php
                                        if(!empty($userPlan) && $userPlan->is_download == 1){
                                            if($audios->aws_upload == 1){
                                                echo  '<li><a href="'.getSongAWSUrlHtml($audios).'"><span class="common_drop_icon drop_downld"></span>'.__("frontWords.download_now").'</a></li>';
                                            }else{
                                                echo '<li><a href="javascript:void(0);" class="download_track" data-musicid="'.$audios->id.'"><span class="common_drop_icon drop_downld"></span>'.__('frontWords.download_now').'</a></li>';
                                            }
                                        }
                                    @endphp 
                                    <li>
                                        <a href="javascript:void;" class="add_to_queue" data-musicid="{{ $audios->id }}" data-musictype="audio">
                                        <span class="opt_icon" title="Add To Queue"><span class="icon icon_queue"></span></span>
                                        {{ __("frontWords.add_to_queue") }}
                                        </a>
                                    </li>
