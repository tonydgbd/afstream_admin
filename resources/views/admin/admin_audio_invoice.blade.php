                        ...
                        <table class="table table-styled mb-0">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <tr>
                                        <td colspan="2">{{ __('adminWords.total_tax') }} 
                                            @if(isset($invoiceData->is_commission) && $invoiceData->is_commission == '1')
                                                @if($invoiceData->commission_type == 'percent')
                                                    {{ '(%)' }}
                                                    <td>{{ $invoiceData->commission.'%' }}</td>
                                                @else
                                                    {{ '' }}
                                                    <td>{{ $invoiceData->commission }}</td>
                                                @endif
                                    ...
                                    <tr>
                                        <td colspan="2">{{ __('adminWords.total_tax_amount') }} :</td>
                                        @php $adminComAmount = 0; @endphp
                                        @if(isset($invoiceData->is_commission) && $invoiceData->is_commission == '1')
                                            @if($invoiceData->commission_type == 'percent')
                                                @php $adminComAmount = ($invoiceData->commission)*$invoiceData->amount/100; @endphp
                                                <td>-{{ ($invoiceData->commission)*$invoiceData->amount/100 }}</td>
                                            @else
                                                @php $adminComAmount = $invoiceData->commission; @endphp
                                                <td>-{{ $invoiceData->commission }}</td>
                                            @endif
                                        @else
                                            <td></td>
                                    ...
                                    <tr>
                                        <td colspan="2"><h5>{{ __('adminWords.total').' '.__('adminWords.amount') }} :</h5></td>
                                        @if(isset($invoiceData->is_commission) && $invoiceData->is_commission == '1')
                                            <td><h5>{{ $invoiceData->amount-$adminComAmount }}</h5></td>
                                        @else
